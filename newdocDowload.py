...
def download_documents(document_ids, document_filenames, output_folder, bearer_token):
    ...
    for doc_id, filename in zip(document_ids, document_filenames):
        ...
        if response.status_code == 200:
            ...
            unique_id = str(uuid.uuid4())[:8]  # Using the first 8 characters of a UUID
            # Append the unique identifier to the filename
            filename_with_id = f'{os.path.splitext(filename)[0]}_{unique_id}{os.path.splitext(filename)[1]}'
            # Save the document to the output folder with the unique filename
            try:
                with open(os.path.join(output_folder, filename_with_id), 'wb') as f:
                    f.write(response.content)
                print(f'Document {filename_with_id} downloaded successfully.')
            except Exception as e:
                print(f'Failed to save document {filename_with_id}: {e}')
